...
class (PurePersistField (AutoKey v), PurePersistField (DefaultKey v)) => PersistEntity v where
  ...
  toEntityPersistValues :: PersistBackendReadOnly m => v -> m ([PersistValue] -> [PersistValue])
  -- | Constructs the value from the list of 'PersistValue'
  fromEntityPersistValues :: PersistBackendReadOnly m => [PersistValue] -> m (v, [PersistValue])
...
class Projection p a | p -> a where
  ...
  projectionResult :: PersistBackendReadOnly m => p -> [PersistValue] -> m (a, [PersistValue])
...
class PersistBackendReadOnly m => PersistBackend m where
  ...
  migrate       :: PersistEntity v => TableAnalysis m -> v -> Migration m
  -- | Execute raw query
...
instance PersistBackendReadOnly m => PersistBackendReadOnly (ReaderT r m) where --TODO: Abstract this newtype-wrapper-monad-stack stuff
...
instance PersistBackend m => PersistBackend (ReaderT r m) where
  insert = lift . insert
...
instance PersistBackendReadOnly m => PersistBackendReadOnly (StateT s m) where
...
instance PersistBackendReadOnly m => PersistBackendReadOnly (Strict.StateT s m) where
...
instance PersistBackendReadOnly m => PersistBackendReadOnly (MaybeT m) where
...
instance PersistBackendReadOnly m => PersistBackendReadOnly (ExceptT e m) where
...
instance (Monoid w, PersistBackendReadOnly m) => PersistBackendReadOnly (CPSWriter.WriterT w m) where
...
class PersistField a where
  ...
  toPersistValues :: PersistBackendReadOnly m => a -> m ([PersistValue] -> [PersistValue])
  -- | Constructs a value from a 'PersistValue'. For complex datatypes it may query the database
  fromPersistValues :: PersistBackendReadOnly m => [PersistValue] -> m (a, [PersistValue])
...
class PersistField a => SinglePersistField a where
  toSinglePersistValue :: PersistBackendReadOnly m => a -> m PersistValue
  fromSinglePersistValue :: PersistBackendReadOnly m => PersistValue -> m a
